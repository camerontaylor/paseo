...
| [side-conversations-merge-blocker.md](side-conversations-merge-blocker.md) | Why `read/side-conversations-phase1-plan` does not merge: upstream #3826 deleted the API its app layer is built on. The collision map, the six conflicts with a resolution each, and the two files that merge clean and then fail to build. Delete when the merge lands. |
