...
| [scripts/](scripts/)                                                                           | Fork-local tooling. `release-fork.mjs` stages, renames and gates a fork npm release; it never publishes without both `--publish` and `--yes-i-am-publishing`. `init-worktree.mjs` brings a transient worktree up in under a second — see [worktrees.md](worktrees.md).                                                                                        |
