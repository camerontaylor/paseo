...
| [side-conversations-follow-ups.md](side-conversations-follow-ups.md)       | Seven follow-ups left on the side-conversations feature after the review fix pass, plus the gotchas that cost time. Delete when the follow-ups are closed.                                                                                                                             |
...
| [side-conversations-merge-blocker.md](side-conversations-merge-blocker.md) | Why `read/side-conversations-phase1-plan` does not merge, and the plan to land it: upstream #3826 drifted the app layer three ways, only one of which produces conflicts. Trial-merge-verified collision map, two decisions, ordered steps, verification. Delete when the merge lands. |
...
| [scripts/](scripts/)                                                       | Fork-local tooling. `release-fork.mjs` stages, renames and gates a fork npm release; it never publishes without both `--publish` and `--yes-i-am-publishing`.                                                                                                                          |
