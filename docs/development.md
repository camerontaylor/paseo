...
**Prerequisites (macOS only).** The service shells out to the Apple toolchain, so beyond the `pnpm install --frozen-lockfile` that worktree setup runs you must install:
...
`serve-sim`, Expo, and Metro come from `pnpm install --frozen-lockfile`, and CocoaPods installs itself on the first prebuild as described above.
...
`patches/*.patch` are applied by pnpm itself, from the `patchedDependencies` map in
`pnpm-workspace.yaml`. A patch is keyed by exact `name@version`, so bumping a patched dependency
fails the install until the key and the patch are updated together — patches can no longer go
stale silently.

Create or edit one with `pnpm patch <name>@<version>`: pnpm checks the package out to a scratch
directory, you edit it there, and `pnpm patch-commit <dir>` writes `patches/<name>@<version>.patch`
and registers it. Do NOT use `patch-package`: it rewrites files inside `node_modules`, which under
pnpm are links into the machine-wide content-addressed store, so it would corrupt every other
project sharing those packages.
...
```bash
pnpm run cli -- ls -a --host localhost:7777
pnpm run cli -- ls -a --host ssh://user@host
```
