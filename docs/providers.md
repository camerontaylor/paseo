...
**Session-scoped cancellation needs a stop boundary inside the provider.** Some agents cancel the whole session rather than one turn — OpenCode's `session.abort` is the example — and some take a cancellation as a notification whose completion proves the write was attempted and nothing else; ACP `session/cancel` is that case. OpenCode and the generic `ACPAgentSession` both implement the boundary below. A cancel that is still in flight when the next run starts will kill that replacement run, which is what makes "stop, then immediately prompt again" (`replaceRunning`, `notifyOnFinish` wakes, schedules) flaky. Own this in the provider session, not in `AgentManager`:
...
- **Install the fence before the first cancellation write.** Admission closes at install, so a replacement that arrives behind the stop queues there instead of reaching the runner, and no later `startTurn` can get in front of the cancel that backs it. ACP installs it in `interrupt()` before `connection.cancel`. A permission denial routes by stop kind instead of always installing: a foreground turn installs the fence and sends the ledger cancel; a provider-owned turn (an ownerless mirror) routes to that provider's own stop; a denial with no stop-able turn suppresses the cancellation write entirely, because a session-scoped cancel with no stopped turn id could kill an unrelated run.
...
- **Open the gate on two facts: the stopped run's authoritative terminal and a settled cancellation ledger.** The terminal has to name the stopped turn, so a terminal belonging to a replacement or to an autonomous wake cannot clear it. The ledger has to be fully settled on the connection the stop captured, with the newest write successful — a rejected newest write keeps the fence closed until a newer one succeeds.
- **Leave room for provider proof on top.** A provider whose own cancel verb does the real stopping can demand stricter evidence than those two facts. GJC's fork-only provider is the example: its ownerless `turn.abort` gate takes a validated terminal-safe abort result for the configured scope plus a post-stop authoritative idle, in either order. The GJC provider is fork-local and this doc is upstream-owned, so its validation matrix lives in `fork/plans/ralplan-gjc-acp-cancellation-boundary.md`.
- **Bound the caller's wait without creating readiness.** OpenCode caps the wait for the previous turn to stop at `OPENCODE_PENDING_ABORT_START_TIMEOUT_MS` (10 s); ACP follows with `ACP_STOPPED_TURN_ADMISSION_TIMEOUT_MS`. Expiry rejects only the queued replacement — no prompt, no turn event, no user echo — and leaves the fence installed with whatever facts are still missing.
- Gate the operations **the daemon issues** (prompt, slash command, summarize) on those two facts. Permission and question responses are not runner operations and must stay outside the gate, or an auto-approve deadlocks the stop. Runs the _provider_ starts on its own — plugin or autonomous wakes — are observed, not gated: the daemon does not choose when they begin, and holding their events back does not protect them from a cancel already in flight, it only hides a run that may already be dead.
